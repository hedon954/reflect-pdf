...
| 🌐 **语境感知翻译**    | 每个语境独立翻译；**流式输出**——首字 0.5–2s 出现，字段一行行填充，不再整段空白等待；选中短语/句子时自动切换句子翻译模式，保存时自动保存到笔记并添加划线标注；LLM 失败时在气泡底部展示具体原因（缺少配置 / 接口报错等），网关挂死时 30s 内自动降级到 MyMemory 兜底 |
...
LLM 流式通过 UniFFI 回调把"已完成的 JSON 字段"即时推给 SwiftUI 主线程，气泡按字段渐进渲染。详见 [v1.0.4 TDD](docs/tdd/tdd-2026-05-06-v104.md)。
