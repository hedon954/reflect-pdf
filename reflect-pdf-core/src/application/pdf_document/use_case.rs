...
impl PdfDocumentUseCase {
    ...
    pub fn upsert(&self, req: UpsertPdfRequest) -> Result<PdfDocument, ReflectError> {
        self.repo.upsert(req)
    }

    pub fn save_reading_position(&self, file_path: &str, page: u32, scroll_offset: f64) -> Result<(), ReflectError> {
        self.repo.save_reading_position(file_path, page, scroll_offset)
    }

    pub fn list(&self) -> Result<Vec<PdfDocument>, ReflectError> {
        self.repo.list()
    }

    pub fn delete(&self, file_path: &str) -> Result<(), ReflectError> {
